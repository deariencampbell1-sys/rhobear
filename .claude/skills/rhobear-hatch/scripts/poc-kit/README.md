...
| `qa_turn16.py` | Ring registration gates (baseline ≤2, feet ≤4, headTop ≤4, height ≤6, synthMidDev ≤4) + annotated 16-cell contact sheet. **Adjudicates**: prints a verdict token to obey — `PASS` / `PASS-WITH-VARIANCE` (canon variance, machine-verified) / `FAIL-SOURCE` (names the broken still) / `FAIL-SYNTH` / `CHECK` (stop+report). Exit codes 0/2/3 for batch drivers. | `python qa_turn16.py --crew <c> --world <w>` |
